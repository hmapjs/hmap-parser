...
Parser.prototype = {
  ...
  initBlock: function(line, nodes) {
    ...
    return {type: 'Block', nodes: nodes, line: line, filename: this.filename};
  },
  ...
  parseExpr: function() {
    switch (this.peek().type) {
      ...
      case 'id':
      case 'class':
        this.tokens.defer({type: 'tag', val: 'div', line: this.peek().line, filename: this.filename});
    ...
    loop : while (true) {
      switch (nextTok.type) {
        case 'text':
          ...
          tags.push({type: 'Text', val: tok.val, line: tok.line, filename: this.filename});
          break;
        case 'interpolated-code':
          var tok = this.advance();
          tags.push({
        ...
        case 'newline':
          ...
          if (this.peek().type === 'text') {
            tags.push({type: 'Text', val: '\n', line: tok.line, filename: this.filename});
          }
          break;
        case 'start-hmap-interpolation':
          this.advance();
          tags.push(this.parseExpr());
          this.expect('end-hmap-interpolation');
          break;
        default:
  ...
  parseWhen: function() {
    ...
    if (this.peek().type !== 'newline') {
      ...
      return {type: 'When', expr: tok.val, debug: false, line: tok.line, filename: this.filename};
    }
  },
  ...
  parseCode: function(noBlock) {
    ...
    assert(typeof tok.mustEscape === 'boolean', 'Please update to the newest version of hmap-lexer.');
    var node = {
      type: 'Code',
      val: tok.val,
      buffer: tok.buffer,
  ...
  parseComment: function() {
    ...
    if (block = this.parseTextBlock()) {
      ...
      return {type: 'Comment', val: tok.val, buffer: tok.buffer, line: tok.line, filename: this.filename};
    }
  },
  ...
  parseDoctype: function() {
    ...
    return {type: 'Doctype', val: tok.val, line: tok.line, filename: this.filename};
  },
  ...
  parseIncludeFilter: function() {
    ...
    return {type: 'IncludeFilter', name: tok.val, attrs: attrs, line: tok.line, filename: this.filename};
  },
  ...
  parseInclude: function() {
    ...
    if ((/\.hmap$/.test(node.file.path)) && !filters.length) {
      node.block = 'indent' == this.peek().type
        ? this.block()
        : this.emptyBlock(tok.line);
      if (/\.jade$/.test(node.file.path)) {
        console.warn(this.filename + ', line ' + tok.line + ':\nThe .jade extension is deprecated, use .hmap for "' + node.file.path + '".');
      }
    } else {
  ...
  tag: function(tag, options) {
    ...
    out : while (true) {
      switch (this.peek().type) {
        ...
        case 'start-attributes':
          if (seenAttrs) {
            console.warn(this.filename + ', line ' + this.peek().line + ':\nYou should not have hmap tags with multiple attributes.');
          }
          seenAttrs = true;
    ...
    switch (this.peek().type) {
      ...
      default:
        ...
        if (pluginResult)
          break;
        this.error('INVALID_TOKEN', 'Unexpected token `' + this.peek().type + '` expected `text`, `interpolated-code`, `code`, `:`' + (selfClosingAllowed
          ? ', `slash`'
          : '') + ', `newline` or `eos`', this.peek())
    }
